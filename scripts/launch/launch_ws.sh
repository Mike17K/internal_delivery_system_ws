...
move_up
paste_cmd 'source install/setup.bash && ros2 launch workcell_bringup workcell.launch.py sim_gazebo:=true use_fake_hardware:=false slam:=false map:=src/robots/agv/agv_navigation/maps/office_world.yaml'

move_right
# planning_bringup (cuMotion, group_a arm planning) was legacy and has been removed
paste_cmd 'ros2 launch agv_navigation teleop.launch.py namespace:=agv_1'
...
move_left
paste_cmd 'ros2 run rviz2 rviz2 --ros-args -r /tf:=/agv_1/tf -r /tf_static:=/agv_1/tf_static'